...
<div class="main">
  <div class="topnav">
     ...
     <div class="nav2">
            ...
            <div class="headtop">
            <a class="productbig" href="product.php?id=23"> <img  src="images/girlsection.png" alt="imagesmid" style="width:60%; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.4);"></a>
            </div>
            <br>
            <br>
            <div class="container">
...
<?php
  ...
  // First query for products with category "Women"
  $queryWomen = "SELECT * FROM webiste.product WHERE product_category = 'Women'";
  if (!empty($brand)) {
      $queryWomen .= " AND brand = '$brand'";
  }
  if (!empty($price)) {
      $queryWomen .= " AND product_price <= '$price'";
  }
  
  $resultWomen = mysqli_query($conn, $queryWomen);
  
  // Second query for products with category "Unisex"
  $queryUnisex = "SELECT * FROM webiste.product WHERE product_category = 'Unisex'";
  if (!empty($brand)) {
      $queryUnisex .= " AND brand = '$brand'";
  }
  if (!empty($price)) {
      $queryUnisex .= " AND product_price <= '$price'";
  }
  
  $resultUnisex = mysqli_query($conn, $queryUnisex);
  
  // Display results for products with category "Women"
  ...
  // Display results for products with category "Unisex"
  while ($row = mysqli_fetch_assoc($resultUnisex)) {
  ?>
    <div class="card">
      <a href="/website/product.php?id=<?php echo $row['productID']; ?>">
        <img src="<?php echo $row["product_img"]; ?>" alt="<?php echo $row["product_name"]; ?>" style="width:100%">
